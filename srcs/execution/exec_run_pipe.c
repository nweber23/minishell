...
/*   Updated: 2025/09/20 13:48:49 by yyudi            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	child_cleanup_and_exit(t_shell_data *sh, t_node *node, int st)
{
...
static pid_t	spawn_left(t_shell_data *sh, t_node *node, int pipe_fds[2])
{
	...
	{
		close(pipe_fds[0]);
		child_exec(sh, node->left, -1, pipe_fds[1]);
		close(pipe_fds[1]);
		_exit(127);
	}
	return (pid);
...
static pid_t	spawn_right(t_shell_data *sh, t_node *node, int pipe_fds[2])
{
	...
	{
		close(pipe_fds[1]);
		child_exec(sh, node->right, pipe_fds[0], -1);
		close(pipe_fds[0]);
		_exit(127);
	}
	return (pid);
